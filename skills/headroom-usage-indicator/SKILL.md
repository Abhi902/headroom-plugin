...
- 🟡 `▲ headroom broken (mcp) · run /doctor` (v2.8) — the engine itself is fine, but the bundled MCP cannot be spawned by the bare name `headroom` (it is not on PATH). Unlike `engine`, this one is **not** cleared by a compression — a working compression proves the engine and says nothing about whether the MCP can be reached — so it persists until `/doctor` resolves it. That is deliberate: filed as `engine` it flapped yellow → normal → yellow every session, for a condition that never changed.
...
**Core principle:** detect real usage from the session transcript, not from intent. It counts `tool_use` calls to `mcp__headroom__headroom_compress` (or the plugin-namespaced `mcp__plugin_<plugin>_headroom__headroom_compress`) plus hcat receipts, and sums the `tokens_saved` / receipt deltas those actually reported — so it can't lie. The dollar figure prices those tokens at the **session model's input rate** (a conservative floor — see below).

## Prerequisites

- The headroom MCP server is available (tools appear as `mcp__headroom__headroom_compress`, `…_retrieve`, `…_stats` for a user-registered server, and as `mcp__plugin_headroom-usage-indicator_headroom__…` for the plugin's bundled one; after `doctor --fix` registers the engine by absolute path both sets can be present — they reach the same engine, either may be called, and the badge counts both). Plugin installs bundle the registration via `.mcp.json`; the engine itself, if missing, can be bootstrapped into `~/.headroom-venv` by the `doctor` skill. Legacy installs must register it by hand. **v2.7.3:** the bundled registration was broken from v2.5 through v2.7.2 — its command carried literal quotes, and MCP `stdio` commands are spawned without a shell, so the launcher was never found (`/plugin` showed a ✗). If those tools are missing on a plugin install, that is the likely cause; `doctor --fix` repairs an installed copy in place.
- `jq` on PATH.
